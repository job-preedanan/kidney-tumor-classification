...
def load_data_from_xlsx(path, file_name, modal_lists, image_type=1, format='crop', num_classes=2):
    ...
    for data in data_list:
        ...
        if image_type == 1:  # image
            ...
            if data[2] == 0:
                img_type_folder = 'AML' if format == 'crop' else 'AML_' + format
            elif data[2] == 1:
                img_type_folder = 'Oncocytoma' if format == 'crop' else 'Oncocytoma_' + format
            elif data[2] == 2:
                img_type_folder = 'CCRCC' if format == 'crop' else 'CCRCC_' + format
            elif data[2] == 3:
                img_type_folder = 'chromophobeRCC' if format == 'crop' else 'chromophobeRCC_' + format
            elif data[2] == 4:
                img_type_folder = 'PapillaryRCC' if format == 'crop' else 'PapillaryRCC_' + format
            ...
            for n in range(num_slice):
                ...
                for m, modal_name in enumerate(modal_lists):
                    ...
                    if format == 'crop':
                        slice_name = \
                        glob.glob1(path + img_type_folder + '/' + id_name + '/' + modal_name + '/', '*.png')[n]
                        image = cv2.imread(path + img_type_folder + '/' + id_name + '/' + modal_name + '/' + slice_name,
                                           cv2.IMREAD_GRAYSCALE)
                    elif format == 'crop_kidney':
                        slice_name = \
                        glob.glob1(path + img_type_folder + '/' + id_name + '/' + modal_name + 'k/', '*.png')[n]
            ...
            for n in range(num_slice):
                ...
                for m, modal_name in enumerate(modal_lists):
                    if format == 'crop':
                        slice_name = glob.glob1(path + img_type_folder + '/' + id_name + '/' + modal_name + 'L/', '*.png')[n]
                        ...
                        slice_name = \
                        glob.glob1(path + img_type_folder + '/' + id_name + '/' + modal_name + 'L/', '*.png')[n]
                        image = cv2.imread(path + img_type_folder + '/' + id_name + '/' + modal_name + 'L/' + slice_name)
...
def create_cropped_kidney_dataset(path):
    ...
    # dataset folder structure : Type / patient_id / modal / slice_no
    for label_folder in ['chromophobeRCC_crop_kidney']:  # 'AML_crop_kidney', 'CCRCC_crop_kidney'
        for id_name in glob.glob1(path + '/' + label_folder + '/', '*'):

            for modal_name in ['pc', 'ec', 'dc', 'tm', 'am']:  #
                for slice_name in glob.glob1(path + '/' + label_folder + '/' + id_name + '/' + modal_name, '*.png'):

                    print(path + '/' + label_folder[:-12] + '/' + id_name + '/' + modal_name + '/' + slice_name)

                    image = cv2.imread(path + '/' + label_folder[:-12] + '/' + id_name + '/' + modal_name + '/' + slice_name)
                    annotated_image = cv2.imread(path + '/' + label_folder + '/' + id_name + '/' + modal_name + 'L/' + slice_name)
                    mask_red = cv2.imread(path + '/' + label_folder[:-12] + '/' + id_name + '/' + modal_name + 'L/' + slice_name)
...
def create_segmented_kidney_dataset(path):
    ...
    def segmentation(img):
        ...
        _, bin_img2 = cv2.threshold(img[:, :, 0], 100, 255, cv2.THRESH_BINARY)

        blurred = cv2.GaussianBlur(img[:, :, 0], (5, 5), 0)
        bin_img = cv2.adaptiveThreshold(blurred, 255,
                                       cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 71, 10)

        bin_img[bin_img2 == 0] = 0

        # cv2.imshow('th', bin_img)
        # cv2.waitKey(0)

        # edges = cv2.Canny(image=blurred, threshold1=50, threshold2=100)
        ...
        # cv2.imshow('erode', bin_img)
        # cv2.waitKey(0)

        # remove small connected components
        # bin_img[edges == 255] = 0

        nb_blobs, im_with_separated_blobs, stats, _ = cv2.connectedComponentsWithStats(bin_img)
        sizes = stats[:, -1]
        sizes = sizes[1:]
        nb_blobs -= 1
        min_size = (img.shape[0] * img.shape[1]) * 0.2    # size < 10% of img size

        # output image with only the kept components
        out = np.zeros_like(im_with_separated_blobs)
        for blob in range(nb_blobs):
            if sizes[blob] >= min_size:
                out[im_with_separated_blobs == blob + 1] = 255

        # fill hole
        out = ndi.binary_fill_holes(out, structure=np.ones((5,5)))

        # clear boader
        out2 = clear_border(out)

        out = out.astype(float)*255
        out2 = out2.astype(float)*255
    ...
    # dataset folder structure : Type / patient_id / modal / slice_no
    for label_folder in ['Oncocytoma_crop_kidney', 'PapillaryRCC_crop_kidney']:  # 'Oncocytoma_crop_kidney', 'CCRCC_crop_kidney'
        for id_name in glob.glob1(path + '/' + label_folder + '/', '*'):

            for modal_name in ['ec', 'dc']:  #
                for slice_name in glob.glob1(path + '/' + label_folder + '/' + id_name + '/' + modal_name, '*.png'):

                    print(path + '/' + label_folder[:-12] + '/' + id_name + '/' + modal_name + '/' + slice_name)

                    crop_k_image = cv2.imread(path + '/' + label_folder + '/' + id_name + '/' + modal_name + 'k/' + slice_name)
                    crop_k_image =segmentation(crop_k_image)

                    # crop_k_mask = cv2.imread(path + '/' + label_folder + '/' + id_name + '/' + modal_name + 'kL/' + slice_name)
                    # full = cv2.imread(path + '/' + label_folder[:-12] + '/' + id_name + '/' + modal_name + '/' + slice_name)
                    ...
                    try:
                        os.makedirs(path + '/' + label_folder + '/' + id_name + '/' + modal_name + '_soft_mask/')
                    except(FileExistsError):
                        print('folders exist')

                    # cv2.imwrite(path + '/' + label_folder + '/' + id_name + '/' + modal_name + '_soft_mask/' + slice_name,
                    #             crop_k_image)
                    # cv2.imwrite(path + '/' + label_folder + '/' + id_name + '/' + modal_name + 'kL/' + slice_name,
                    #             crop_mask)


                    cv2.imshow('image', crop_k_image)
                    cv2.waitKey(100)
                    #
                    # cv2.imshow('mask', crop_mask)
                    # cv2.waitKey(5)
...
if __name__ == '__main__':
    ...
    # print(images.shape)
    # print(labels.shape)
    create_segmented_kidney_dataset('C:/Users/Job/Documents/DoctorProject/kidney_tumor/dataset')
