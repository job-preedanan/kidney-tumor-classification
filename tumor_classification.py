...
DATA_FOLDER = '/dataset/'
EXPORT_FOLDER = '/exports/classification/cropped_tumor/dataset2023/5_classes/3_modal/pretrained_vgg16_focal_loss/'               #_
IMAGE_SIZE = 224
NUM_CHANNELS = 3
BATCH_SIZE = 16
EPOCH = 100
...
def focal_loss(gamma=2., alpha=.25):
    def focal_loss_fixed(y_true, y_pred):
        ...
        return -tf.reduce_mean(alpha * tf.pow(1. - pt_1, gamma) * tf.math.log(pt_1+0.000001)) - tf.reduce_mean((1 - alpha) * tf.pow(pt_0, gamma) * tf.math.log(1. - pt_0 + 0.000001))
...
def train(x_train, y_train, x_val, y_val, modal_name, mulmonet=False):
    ...
    if mulmonet:
        ...
        # model = make_mulmoNet_vgg16(3, IMAGE_SIZE, IMAGE_SIZE, 3, 1)
        # model.summary()
        ...
        model = make_pretrained_model(IMAGE_SIZE, IMAGE_SIZE, 3, 5)
        model.summary()
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.001),
                  loss=focal_loss(),  # tf.keras.losses.CategoricalCrossentropy()
                  metrics=['accuracy'])
    ...
    if mulmonet:
        ...
        # fits the model on batches with real-time data augmentation:
        history = model.fit(generator(x_train1, train_label, batch_size=BATCH_SIZE),
                            validation_data=(x_val1, val_label),
...
def predict(x_test, y_test, modal_name, mulmonet=False):
    ...
    if mulmonet:
        ...
        # prediction
        y_pred_logit = model.predict([x_test1, x_test2, x_test3], BATCH_SIZE)
    ...
    if NUM_CLASSES == 5:
        label_name = ['AML', 'Oncocytoma', 'CCRCC', 'chromophobeRCC', 'PapillaryRCC']
        ...
        for i in range(NUM_CLASSES):
            ...
            plt.xlabel('FPR')
            plt.ylabel('TPR')
            plt.title('ROC of class#' + label_name[i])
            plt.legend(loc="lower right")
            plt.savefig(PATH + EXPORT_FOLDER + modal_name + '/cross_val#' + str(CROSS_NUM) + '/ruc_curve_' + label_name[i] + '.png')
            plt.clf()

        # transform one-hot to label name
        y_pred_label = []
        y_test_label = []
        for i in range(len(y_pred_logit)):
            y_pred_label.append(label_name[np.argmax(y_pred_logit[i])])
            y_test_label.append(label_name[np.argmax(y_test[i])])

        # confusiion matrix
        confusion_matrix = confusion_matrix(y_test_label, y_pred_label, labels=label_name)
        print(confusion_matrix)

        df_cm = pd.DataFrame(confusion_matrix, index=label_name, columns=label_name)
        ...
        print('\nClassification Report\n')
        report = classification_report(y_test_label, y_pred_label, target_names=label_name, output_dict=True)
        print(report)
        ...
        df.to_excel(PATH + EXPORT_FOLDER + modal_name + '/cross_val#' + str(CROSS_NUM) + '/evaluation_results.xlsx',
...
if __name__ == '__main__':
    ...
    for modal_name in modal_lists:
        ...
        # idx = random.randint(0, len(x_train_val))
        # cv2.imshow('img', utils.denormalize_x(x_train_val[idx]))
        # cv2.waitKey(0)

        # for m, modal in enumerate(modal_lists):
        #     axarr[0, m].set_title(modal)
        #     axarr[0, m].grid(False)
        #     axarr[0, m].imshow(utils.denormalize_x(x_train_val[idx, :, :, m*3:m*3 + 3]), cmap='gray', vmin=0, vmax=255)
        #     axarr[1, m].imshow(utils.denormalize_y(x_train_val[idx, :, :, m]), cmap='gray', vmin=0, vmax=255)
        #
        # plt.show()
        ...
        for CROSS_NUM in range(5):
            print('Cross_val ## ' + str(CROSS_NUM+1) + '/' + str(5))
            ...
            if CROSS_NUM >= 0:
                if NUM_CLASSES == 2:
                    ...
                    print('train : #Benign = ' + str(len(y_train[y_train == 0])) + ', #Malignant = ' + str(
                        len(y_train[y_train == 1])))
                    print('val : #Benign = ' + str(len(y_val[y_val == 0])) + ', #Malignant = ' + str(
                        len(y_val[y_val == 1])))
                    print('test : #Benign = ' + str(len(y_test[y_test == 0])) + ', #Malignant = ' + str(
                        len(y_test[y_test == 1])))
        ...
        if NUM_CLASSES == 2:
            ...
            excel_pixel.to_excel(PATH + EXPORT_FOLDER + modal_name[0] + '_' + modal_name[1] + '_' + modal_name[2] + '/evaluation_results.xlsx',
